...
interface ReconciliationState {
  ...
  calculateTotalDifference: () => void;
}

export const reconciliationStore = create<ReconciliationState>((set, get) => ({
  dataCollection: [],
  totalDoctos: 0,
  totalCash: 0,
  totalDifference: 0,
  ...
  calculateTotalDifference: () => {
    const cash = get().totalCash;
    const doctos = get().totalDoctos;
    const checkout = get().totalCheckout
    set({ totalDifference: (checkout - (cash + doctos))})
  }  
}));
